...
export async function POST(req) {
    try {
        const { oldMobile, newMobile } = await req.json();
        ...
        if (!externalApiBaseUrl && !process.env.MOBILE_UPDATE_OTP_API_URL) {
        ...
        const externalApiUrl =
            process.env.MOBILE_UPDATE_OTP_API_URL ||
            `${externalApiBaseUrl.replace(/\/+$/, "")}/candidate/candi/sendMobileUpdateOtp`;
        ...
        return NextResponse.json(
            { message: externalData.message || (failed ? "Unable to send OTP." : "OTP sent successfully.") },
            { status: failed ? (externalResponse.ok ? 400 : externalResponse.status) : 200 }
        );
    } catch (error) {
        console.error("CANDIDATE MOBILE OTP ERROR:", error);
