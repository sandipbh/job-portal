...
export async function POST(req) {
  ...
  try {
    ...
    const externalApiUrl =
      ...
      `${externalApiBaseUrl.replace(/\/+$/, "")}/api/public/updateProfileCount`;
    ...
    if (!externalResponse.ok) {
      console.error(
        "External Update Profile view failed:",
    ...
    const response = NextResponse.json(
      {
        message: responseData.message || "Profile view saved successfully",
      },
      { status: 201 }
    );
    ...
    return NextResponse.json(
      { message: "Failed to save profile view" },
      { status: 500 }
    );
  }
}