...
export async function GET(req) {
  ...
  try {
    ...
    const externalApiUrl =
      ...
      `${externalApiBaseUrl.replace(/\/+$/, "")}/Candidate/candi/getSkills`;
...
export async function POST(req) {
  ...
  try {
    ...
    const externalApiUrl =
      process.env.REGISTER_API_URL ||
      `${externalApiBaseUrl.replace(/\/+$/, "")}/Candidate/candi/saveSkills`;
    ...
    console.log("Skills details successfully");

    // 6. Send response with cookies
    const response = NextResponse.json(
      {
        message: responseData.message || "Skills details successfully",
      },
      { status: 201 }
    );
